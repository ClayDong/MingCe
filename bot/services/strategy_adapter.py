...
使用方式:
    ...
    signals = await adapter.get_signals_async(["SZ002594", "SH600519"])
    all_signals = await adapter.get_all_signals_async()

    # 同步接口（向后兼容，内部用 asyncio.to_thread 包装）
...
class StrategySignalAdapter:
    ...
    async def _run_http_async(self, symbols: list[str]) -> Optional[dict]:
        """通过 HTTP 异步调用信号服务（不阻塞事件循环）"""
        try:
            import httpx

            payload = {"symbols": symbols}
            async with httpx.AsyncClient(timeout=_SIGNAL_SERVICE_TIMEOUT) as client:
            ...
            logger.warning(f"HTTP 异步调用信号服务失败: {e}")
            return None

    async def _run_http_all_async(self) -> Optional[dict]:
        """异步获取所有自选股信号"""
        try:
            symbols = await self._get_watchlist_from_signals_async()
            if not symbols:
                logger.warning("无法获取自选股列表，HTTP 模式回退到 subprocess")
                return None
            return await self._run_http_async(symbols)
        except Exception as e:
            logger.warning(f"HTTP 异步获取全部信号失败: {e}")
            return None

    async def _get_watchlist_from_signals_async(self) -> list[str]:
        """异步获取自选股列表（优先异步，回退到线程池读 DB）"""
        # 方式1: 通过 portfolio_manager 异步获取
        try:
            ...
            logger.debug(f"portfolio_manager 异步获取自选股失败: {e}")

        # 方式2: 回退到线程池读取 SQLite（避免阻塞事件循环）
        try:
            return await asyncio.to_thread(self._get_watchlist_from_db)
        except Exception as e:
            logger.debug(f"线程池读取 portfolio.db 失败: {e}")

        logger.warning("⚠️ 无自选股数据，策略信号将为空")
        return []

    def _get_watchlist_from_db(self) -> list[str]:
        """同步读取 SQLite 自选股（仅在线程池中调用）"""
        portfolio_db = Path(__file__).parent.parent / "data" / "portfolio.db"
        ...
        try:
            ...
            logger.debug(f"读取 portfolio.db 失败: {e}")
            return []

    # ── 旧同步接口（向后兼容，内部用线程池包装） ────────

    def _run_http_sync(self, symbols: list[str]) -> Optional[dict]:
        """通过 HTTP 调用信号服务（同步封装，仅向后兼容）"""
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # 在事件循环中：用 run_coroutine_threadsafe 避免阻塞
                future = asyncio.run_coroutine_threadsafe(
                    self._run_http_async(symbols), loop
                )
                return future.result(timeout=_SIGNAL_SERVICE_TIMEOUT + 5)
            return loop.run_until_complete(self._run_http_async(symbols))
        except Exception as e:
            logger.warning(f"HTTP 同步调用信号服务失败: {e}")
            return None

    def _run_http_all_sync(self) -> Optional[dict]:
        """通过 HTTP 获取所有自选股信号（同步封装）"""
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                future = asyncio.run_coroutine_threadsafe(
                    self._run_http_all_async(), loop
                )
                return future.result(timeout=_SIGNAL_SERVICE_TIMEOUT + 5)
            return loop.run_until_complete(self._run_http_all_async())
        except Exception as e:
            logger.warning(f"HTTP 同步获取全部信号失败: {e}")
            return None
    ...
    def _get_watchlist_from_signals(self) -> list[str]:
        ...
        try:
            ...
            if loop.is_running():
                future = asyncio.run_coroutine_threadsafe(
                    self._get_watchlist_from_signals_async(), loop
                )
                return future.result(timeout=10)
            return loop.run_until_complete(self._get_watchlist_from_signals_async())
        except Exception as e:
            logger.debug(f"同步获取自选股失败: {e}")
            # 最终回退到直接读 DB
            return self._get_watchlist_from_db()

    # ── subprocess 调用（回退，异步包装） ───────────────

    async def _run_subprocess_async(self, *args) -> Optional[dict]:
        """异步执行 subprocess（在线程池中运行，不阻塞事件循环）"""
        return await asyncio.to_thread(self._run_subprocess, *args)
    ...
    async def _run_async(self, *args) -> Optional[dict]:
        """异步运行策略分析（不阻塞事件循环）
        ...
        """
        if _FALLBACK_MODE:
            logger.debug("使用 subprocess 回退模式（异步）")
            return await self._run_subprocess_async(*args)
        ...
        if is_list_mode or not symbols:
            watchlist = await self._get_watchlist_from_signals_async()
            if not watchlist:
                logger.warning("无自选股列表，回退到 subprocess")
                return await self._run_subprocess_async(*args)
            http_result = await self._run_http_async(watchlist)
            if http_result is not None:
                logger.info("✅ 使用 HTTP 异步模式获取全部信号成功")
                return self._format_list_response(http_result)
            logger.warning("HTTP 异步模式获取全部信号失败，回退到 subprocess")
            return await self._run_subprocess_async(*args)

        http_result = await self._run_http_async(symbols)
        if http_result is not None:
            logger.info(f"✅ 使用 HTTP 异步模式分析 {len(symbols)} 只股票成功")
            return self._format_analyze_response(http_result, symbols)
        logger.warning("HTTP 异步模式分析失败，回退到 subprocess")
        return await self._run_subprocess_async(*args)
    ...
    async def get_signals_async(self, symbols: list[str]) -> dict:
        """异步获取指定股票列表的策略信号
        ...
        result = await self._run_async(symbols_str)
