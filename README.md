...
> **触发式推送**：当市场出现重大异动（至少2个不同类别触发条件同时满足），自动推送"炒股的智慧·深度分析"卡片
...
│   │   └── wisdom_analyzer.py # 炒股的智慧深度分析
│   ├── models/
│   │   └── schemas.py         # Pydantic 数据模型
│   ├── skills/
│   │   └── wisdom/           # 《炒股的智慧》蒸馏知识库（7个SKILL）
│   ├── tests/                 # 143 个测试用例
│   ├── Dockerfile
│   ├── requirements.txt
...
| `API_KEY` | ❌ | API 认证密钥，保护写端点 |
| `DEEPSEEK_API_KEY` | ❌ | DeepSeek API 密钥（深度分析专用，基于《炒股的智慧》知识库） |
