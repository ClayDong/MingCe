...
| 🧠 **AI 解读** | LLM 基于《炒股的智慧》交易原则，大市判断+临界点四阶段+三层过滤 | LLM (Qwen3-8B) |
| 🔮 **炒股的智慧·深度分析** | 基于陈江挺《炒股的智慧》7个决策SKILL，触发式深度市场分析 | wisdom_analyzer |
...
> ⚡ **非定时，触发式**：当市场出现显著异动时自动触发推送，无固定时间。

| 特征 | 说明 |
|:-----|:------|
| **触发机制** | 5类触发条件检测（市场周期触发/入场信号触发/风险预警触发/心理触发/泡沫触发），至少2个不同类别同时满足才启动 |
| **触发条件示例** | 大市趋势反转信号、临界点突破/回踩确认、止损线触及、心理陷阱征兆（贪婪/恐惧/犹豫）、泡沫特征识别等 |
| **平静模式** | 市场平静时推送"平静模式"通知，告知用户当前无显著异动 |
| **飞书指令** | `@机器人 深度分析` 可手动触发 |
| **卡片样式** | 紫色主题卡片，包含触发条件摘要 + 深度分析正文 + 出处说明 + 免责声明 |
...
| **深度分析** | market-daily-bot | wisdom_analyzer，触发式推送 |
...
每个数据项支持主源 + 多级回退，确保单数据源故障时系统持续可用，当前数据完整率达 **90%**（仅美元指数因网络限制无法获取）：
...
| `深度分析` | 手动触发深度市场分析 | `深度分析` |
| `帮助` | 显示所有指令 | `帮助` |

### 4.6 炒股的智慧·深度分析 — `services/wisdom_analyzer.py`

> **知识来源**：陈江挺《炒股的智慧》蒸馏知识库（books2skill），包含7个决策SKILL。

- **分析框架来源**：陈江挺《炒股的智慧》蒸馏知识库（books2skill），包含7个决策SKILL：
  - **大市判断**：判断市场整体趋势与所处阶段
  - **临界点四阶段**：蓄势→突破→回踩→加速的临界点识别
  - **三层过滤入场**：大市→板块→个股的三层筛选入场机制
  - **止损铁律**：严格止损纪律，保本第一
  - **让利润奔跑**：盈利持仓不急于兑现，用移动止损保护利润
  - **分层下注**：试探性建仓→确认加仓→趋势满仓的资金管理
  - **六大心理陷阱**：贪婪/恐惧/犹豫/后悔/侥幸/从众的心理自检

- **知识库加载**：从 `bot/skills/wisdom/` 目录加载7个SKILL.md（入场决策/止损决策/仓位管理/止盈决策/移动止损/心理自检/泡沫参与），提取I段方法论+E段执行步骤+B段边界

- **数据来源**：复用 `data_fetcher` 采集的真实市场数据（akshare），数据本身是真实的

- **触发机制**：
  - 5类触发条件检测：市场周期触发 / 入场信号触发 / 风险预警触发 / 心理触发 / 泡沫触发
  - 至少2个不同类别同时满足才启动深度分析
  - 触发条件示例：大市趋势反转信号、临界点突破/回踩确认、止损线触及、心理陷阱征兆（贪婪/恐惧/犹豫）、泡沫特征识别等
  - 市场平静时推送"平静模式"通知

- **输出格式**：
  - 大市与阶段判断 + 五维交叉验证 + 操作建议 + 心理提醒
  - 每个关键论据标注出处：`[数据: 来源]` + `[框架: 《炒股的智慧》第X章·XXX]`
...
- **API端点**：`POST /api/wisdom/analyze`（需 API Key 认证）
- **飞书指令**：`@机器人 深度分析`
...
| **v3.1** | **2026-06-23** | **炒股的智慧·深度分析**：基于陈江挺《炒股的智慧》7个决策SKILL / 5类触发条件检测（市场周期/入场信号/风险预警/心理/泡沫） / 触发式深度市场分析 / 紫色主题飞书卡片 / 平静模式 / API端点 `POST /api/wisdom/analyze` / 飞书指令 `@机器人 深度分析` |
