...
数据采集采用多源回退策略，确保单数据源故障时系统持续可用：

| 等级 | 数据维度 | 首选源 | 回退1 | 回退2 | 回退3 |
|:-----|:---------|:-------|:------|:------|:------|
| 个股K线 | 自选股日线 | 新浪财经 HTTP API | 腾讯财经 API | yfinance | 缓存历史 |
| A股指数 | 上证/深证等 | akshare `ak.stock_zh_index_daily()` | 新浪逐只获取 | — | — |
| 美股指数 | 标普/道指/纳指 | 新浪逐只获取 | — | — | — |
| 宏观(金/油) | 伦敦金/布伦特 | akshare 期货历史 | 新浪财经 | — | — |
| 汇率 | 美元指数/主要汇率 | akshare `ak.spot_quote()` | 新浪财经 | — | — |
| 板块轮动 | 行业板块涨跌 | akshare `ak.stock_board_industry_hist()` | — | — | — |
| 北向资金 | 沪深港通流向 | akshare `ak.stock_hsgt_north_net_flow_in_hist()` | — | — | — |
