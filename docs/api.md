...
- **HTTP 方法**：`POST`
- **路径**：`/api/wisdom/analyze`
- **认证**：需要 API Key（请求头 `X-API-Key`）
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| use_existing_data | boolean | 否 | 是否复用已有日报数据，默认 true |
...
```bash
curl -X POST http://localhost:8000/api/wisdom/analyze \
  -H "X-API-Key: your-api-key"
```

#### 注意事项

- 该接口需要 API Key 认证，未配置 API_KEY 环境变量时所有写端点不受限制。
- 深度分析是异步任务，提交后通过 /api/task/{task_id} 查询进度。
- 市场平静时自动跳过，返回 skipped 状态并推送"平静模式"飞书通知。
- 触发条件：至少2个不同类别的市场异动信号同时满足。
...
| `POST /api/wisdom/analyze` | `POST /api/v1/wisdom/analyze` |

> **建议**：新开发请使用不带 `/v1` 的最新路径。v1 路由仅为向后兼容保留，未来版本可能移除。

---

> 文档版本：v1.0.0 | 最后更新：2026-06-23
